...
## `auth/offlineTokenProvider.{js,d.ts}` are generated, not committed

The provider's compiled output used to be tracked and regenerated by hand with `npm run build:auth`,
which nothing invoked. That could not hold, for a reason worth stating precisely: the pre-commit
**prettier** hook reformats the file after `tsc` writes it — single quotes and tabs, against tsc's
double quotes and four spaces — so a committed copy is _structurally guaranteed_ to differ from a
fresh build. A "regenerate and diff" CI guard would therefore fail on a correct tree, and the usual
response to a gate that is red for correct code is to delete it. Measured rather than argued: at
8.4.0 the tracked `.js` had already drifted from its source (11 438 → 12 409 bytes, formatting only).

They are now `.gitignore`d and produced by CI (`.github/workflows/tests.yml`, step
"Build the auth provider (declaration emit)").

Nothing regressed by untracking them, because nothing consumed them:

- `pretest` compiles the provider **and** its spec into `.test-build/` (gitignored) and `npm test`
  runs c8 over that copy — the `**/offlineTokenProvider.js` include glob resolves there, and c8
  without `--all` only reports files that were actually loaded.
- `examples/loginAndListCallers.ts` and the spec import `./offlineTokenProvider`, i.e. the **`.ts`**.
- `make create_npm_package` copies `api`, `public-api.*`, `package.json`, `package-lock.json`,
  `LICENSE`, `README.md` — never `auth/`. Confirmed against the published 8.4.0 tarball: it contains
  no `auth/offlineTokenProvider.*` (the `api/google/api/auth_*` entries are unrelated protos).
- `eslint.config.mjs` already ignored both files as compiled output.

The CI step is **not** redundant with `npm test`: `pretest` compiles without `--declaration`, so the
`.d.ts` emit — which can fail on its own when an inferred type is not portable — is exercised only
there.

**Consequence worth knowing:** the offline-token provider is not part of the published npm package at
all, so a consumer copying `examples/loginAndListCallers.ts` cannot import it. Shipping it would mean
adding `auth/` to `create_npm_package` and exporting it from `public-api`, which is a public-API
...
```text
Release ONDEWO VTSI Nodejs Client ${ONDEWO_VTSI_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO VTSI Nodejs Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO VTSI Nodejs Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/vtsi-client-nodejs`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/vtsi-client-nodejs versions --json
```
